      ...
      <header className="app-header">
        ...
        <div className="header-actions">
          ...
          <div className="draft-menu-wrap">
            <button className="button secondary" type="button" onClick={() => { setDraftsOpen((open) => !open); setSavedInvoicesOpen(false); }} aria-expanded={draftsOpen}>
              Drafts <span className="count">{drafts.length}</span>
            </button>
            {draftsOpen && (
              <div className="draft-menu" aria-label="Saved drafts">
                <div className="draft-menu-head"><strong>Local drafts</strong><button type="button" onClick={() => newInvoice()}>+ New</button></div>
                {drafts.length === 0 ? <p className="empty-state">No saved drafts yet.</p> : drafts.map((draft) => (
                  <div className="draft-row" key={draft.invoice.id}>
                    <button type="button" className="draft-load" onClick={() => loadDraft(draft)}>
                    ...
                    <button className="icon-button" type="button" aria-label={`Delete ${draft.invoice.invoiceNumber || "draft"}`} onClick={() => removeDraft(draft.invoice.id)}>×</button>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="draft-menu-wrap">
            <button className="button secondary" type="button" onClick={() => { setSavedInvoicesOpen((open) => !open); setDraftsOpen(false); }} aria-expanded={savedInvoicesOpen}>
              Invoices <span className="count">{savedInvoices.length}</span>
            </button>
            {savedInvoicesOpen && (
              <div className="draft-menu invoice-menu" aria-label="Saved invoices">
                <div className="draft-menu-head"><strong>Saved invoices</strong><button type="button" onClick={() => newInvoice()}>+ New</button></div>
                {savedInvoices.length === 0 ? <p className="empty-state">No saved invoices yet.</p> : savedInvoices.map((record) => (
                  <div className="invoice-record-row" key={record.invoice.id}>
                    <button type="button" className="draft-load" onClick={() => loadSavedInvoice(record)}>
                    ...
                    <select
                      ...
                      value={record.status}
                      onChange={(event) => changeInvoiceStatus(record, event.target.value as InvoiceStatus)}
                    >
                      {invoiceStatuses.map((status) => <option value={status} key={status}>{invoiceStatusLabels[status]}</option>)}
                    </select>
                    <button className="icon-button" type="button" aria-label={`Delete invoice ${record.invoice.invoiceNumber}`} onClick={() => deleteSavedInvoice(record)}>×</button>
                  </div>
                ))}
              </div>
            )}
          </div>
          <button className="button primary header-save" type="button" onClick={saveInvoiceToDatabase} disabled={isSavingInvoice}>
            {isSavingInvoice ? "Saving…" : "Save"}
          </button>
          <button className="button secondary header-download" type="button" onClick={exportPdf} disabled={isExporting}>
